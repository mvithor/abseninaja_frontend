...
const JadwalMapelList = () => {
  ...
  // 🔎 helper pencarian yang aman null
  const s = (v) => (v ?? '').toString().toLowerCase();

  // 🗓️ urutan hari untuk sorting
  const dayOrder = {
    'Senin': 1, 'Selasa': 2, 'Rabu': 3, 'Kamis': 4, 'Jumat': 5, 'Sabtu': 6, 'Minggu': 7
  };
  const hariIndex = (h) => dayOrder[h] || 99;

  // ⏰ ambil jam mulai untuk sorting (format "HH:MM:SS - HH:MM:SS")
  const getStartTime = (w) => (w || '').split(' - ')[0] || '';

  // 🔁 filter + sort: cari di kelas/mapel/guru/kategori/hari/waktu/kode_offering
  const filteredJadwalMapel = jadwalMapel
    .filter((row) => {
      const q = s(searchQuery);
      return (
        s(row.kelas).includes(q) ||
        s(row.mata_pelajaran).includes(q) ||
        s(row.nama_guru).includes(q) ||
        s(row.kategori).includes(q) ||
        s(row.hari).includes(q) ||
        s(row.waktu).includes(q) ||
        s(row.kode_offering).includes(q)
      );
    })
    .sort((a, b) => {
      const ha = hariIndex(a.hari);
      const hb = hariIndex(b.hari);
      if (ha !== hb) return ha - hb;
      const ta = getStartTime(a.waktu);
      const tb = getStartTime(b.waktu);
      // "HH:MM:SS" lexicographically works for time ordering
      if (ta < tb) return -1;
      if (ta > tb) return 1;
      // fallback: kelas lalu mapel
      return s(a.kelas).localeCompare(s(b.kelas)) || s(a.mata_pelajaran).localeCompare(s(b.mata_pelajaran));
    });
      ...
      <ParentCard title="Jadwal Mata Pelajaran">
        ...
        >
          <SearchButton
            ...
            placeholder="Cari Kelas / Mapel / Guru / Kategori / Kode Offering"
          />
          <AddButton
            icon={<IconPlus size={20} color="white" />}
            onClick={handleAdd}
          >
            {/* 🔤 lebih pas sesuai konsep: Tambah Jadwal */}
            Tambah Jadwal
          </AddButton>
          <FilterButton />
        </Box>
