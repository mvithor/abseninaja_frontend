    ...
    useEffect(() => {
        if (data) {
            setMapelData({
                ...
                semester_mode: data.semester_mode,
                active_semester: data.active_semester,
                kelas_terdaftar: data.kelas_terdaftar || [],
                kelas_ids: [],
            });

            if (Array.isArray(data.kelas_terdaftar) && Array.isArray(kelasOptions) && kelasOptions.length > 0) {
                const preset = kelasOptions.filter(opt =>
                    data.kelas_terdaftar.some(k => k.nama_kelas === opt.nama_kelas)
                );
                setSelectedKelas(preset);
                setMapelData(prev => ({ ...prev, kelas_ids: preset.map(x => x.id) }));
            }
        }
    }, [data, kelasOptions]);
        ...
        <PageContainer title="Edit Mata Pelajaran" description="Edit Mata Pelajaran">
            <ParentCard title="Form Edit Mata Pelajaran">
                ...
                <MataPelajaranEditForm
                    ...
                    isLoading={isFetching || mutation.isLoading || isKelasLoading}
