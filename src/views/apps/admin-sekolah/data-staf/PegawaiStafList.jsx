...
const PegawaiStafList = () => {
  ...
  const { data: staf = [], isLoading, isError, error: queryError } = useQuery({
    ...
    onError: (err) => {
      // react-query error di sini biasanya sudah Error(message),
      // jadi jangan berharap err.response selalu ada.
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const response = await axiosInstance.delete(`/api/v1/admin-sekolah/staf/${id}`);
      return response.data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries(['pegawaiStaf']);
      setSuccess(data?.msg || "Pegawai staf berhasil dihapus");
      setTimeout(() => setSuccess(""), 3000);
    },
    onError: (err) => {
      const msg = err?.response?.data?.msg || err?.message || "Terjadi kesalahan saat menghapus data";
      setError(msg);
      setTimeout(() => setError(""), 3000);
    }
  });
  ...
  const filteredPegawaiStaf = useMemo(() => {
