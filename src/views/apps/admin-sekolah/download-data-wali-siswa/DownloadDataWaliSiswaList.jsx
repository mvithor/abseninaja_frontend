...
import DownloadDataWaliSiswaTable from "src/apps/admin-sekolah/download-data-wali-siswa/DownloadDataWaliSiswaTable";
import { useQuery } from "@tanstack/react-query";
import axiosInstance from "src/utils/axiosInstance";

const fetchKelas = async () => {
  try {
    const response = await axiosInstance.get('/api/v1/admin-sekolah/wali-siswa/kelas');

    const payload = response?.data || {};
    const items = payload?.items || payload?.data || payload?.data?.items || payload?.data || [];
...
const DownloadDataWaliSiswaList = () => {
  ...
  const filteredKelas = (Array.isArray(kelas) ? kelas : [])
    .map((k) => {
      const id = k?.kelas_id ?? k?.id ?? null;
      const nama_kelas = k?.nama_kelas ?? '';
      return {
        ...k,
        id,
        nama_kelas,
        jumlah_wali: k?.jumlah_wali ?? k?.jumlah ?? 0,
        normalizedNamaKelas: normalizeNamaKelas(nama_kelas),
      };
    })
    .filter((k) =>
  ...
  const handleDownload = async (kelasId) => {
    try {
      ...
      const response = await axiosInstance.get(`/api/v1/admin-sekolah/download/data-wali-siswa`, {
      ...
      const filename =
        ...
        `Data_Wali_Siswa_${normalizeNamaKelas(kelasRow?.nama_kelas || 'Kelas')}.pdf`;
  ...
  return (
    <PageContainer title="Download Data Wali Siswa" description="Download Data Wali Siswa per Kelas">
      <ParentCard title="Download Data Wali Siswa">
        ...
        <DownloadDataWaliSiswaTable
          waliData={filteredKelas}
          page={page}
...
export default DownloadDataWaliSiswaList;
