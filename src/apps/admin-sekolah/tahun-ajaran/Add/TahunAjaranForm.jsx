...
const TahunAjaranForm = ({ setSuccess, setError }) => {
  ...
  const mutation = useMutation({
    mutationKey: ["tambahTahunAjaran"],
    mutationFn: async (newTahunAjaran) => {
      const response = await axiosInstance.post("/api/v1/admin-sekolah/tahun-ajaran", newTahunAjaran);
    ...
    onError: (error) => {
      ...
      const errorMsg = error.response?.data?.msg || "Terjadi kesalahan saat menambahkan data tahun ajaran";
  ...
  const handleSubmit = (event) => {
    ...
    mutation.mutate({
      tahun_ajaran: formState.tahun_ajaran,
