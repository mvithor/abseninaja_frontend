...
const QRCodeScanner = ({ onScanSuccess, onScanError, setErrorGlobal, disabled = false }) => {
  ...
  useEffect(() => {
    try {
      ...
      // localStorage tidak tersedia (private browsing / SSR) — diabaikan
    }
  }, []);
  ...
  const playErrorSound = async () => {
    try {
      ...
      // autoplay policy browser — diabaikan, sound tidak kritis
    }
  };
  ...
  useEffect(() => {
    ...
    const onDeviceChange = async () => {
      try {
        ...
        // enumerateDevices gagal (permission dicabut saat runtime) — diabaikan
      }
    };
        ...
        <FormControl fullWidth sx={{ maxWidth: 400, mb: 2 }}>
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              try { localStorage.setItem(STORAGE_KEY, id); } catch { /* private browsing — diabaikan */ }
