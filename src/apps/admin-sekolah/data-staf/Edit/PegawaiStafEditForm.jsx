...
const PegawaiStafEditForm = ({ stafData, handleChange, handleSubmit, handleCancel, isLoading }) => {
  ...
  const { data: subKategoriOptions = [], isLoading: isSubKategoriLoading } = useQuery({
    queryKey: ["subKategoriOptions", stafData.kategori_pegawai_id],
    queryFn: async () => {
      if (!stafData.kategori_pegawai_id) return [];
      const response = await axiosInstance.get(`/api/v1/admin-sekolah/kategori-pegawai/${stafData.kategori_pegawai_id}/subkategori`);
      return response.data.data.sort((a, b) =>
        a.nama_subkategori.localeCompare(b.nama_subkategori)
      );
    },
    enabled: !!stafData.kategori_pegawai_id,
  });

  const safeSubkategoriValue = subKategoriOptions.some(
    (option) => option.id === stafData.subkategori_pegawai_id
  ) ? stafData.subkategori_pegawai_id : "";
      ...
      <Box sx={{ borderRadius: 2, mt: 0 }}>
        <Grid container spacing={2} rowSpacing={1}>
          <Grid size={{ xs: 12, md: 6 }}>
            <CustomFormLabel htmlFor="name" sx={{ mt: 4 }}>Nama Staf</CustomFormLabel>
            <CustomOutlinedInput
              id="name"
              name="name"
              value={stafData.name || ""}
          ...
          {/* NOTE: backend kamu minta current_password saat email berubah */}
          <Grid size={{ xs: 12, md: 6 }}>
            <CustomFormLabel htmlFor="current_password" sx={{ mt: 1.85 }}>
              Password Saat Ini (wajib jika email diubah)
            </CustomFormLabel>
            <CustomOutlinedInput
              id="current_password"
              name="current_password"
              type="password"
              value={stafData.current_password || ""}
              onChange={handleChange}
              fullWidth
              readOnly
            />
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            <CustomSelect
              ...
              value={stafData.kategori_pegawai_id || ""}
              onChange={handleChange}
              fullWidth
            >
              {isKategoriLoading ? (
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            <LocalizationProvider dateAdapter={AdapterDateFns}>
              <DatePicker
                value={stafData.tanggal_lahir ? new Date(stafData.tanggal_lahir) : null}
                onChange={(date) =>
...
export default PegawaiStafEditForm;
