...
const PpdbPeriodEditForm = ({
  ...
  const status = upper(periodData?.status);
  const isDraft = status === "DRAFT";
  const isOpen = status === "OPEN";
  const canEditOpenAt = isDraft;          
  const canEditCloseAt = isDraft || isOpen;
  const canSubmit = isDraft || isOpen;

  const { data: tahunAjaranOptions = [], isLoading: isTahunAjaranLoading, isError: tahunAjaranError } = useQuery({
    queryKey: ["tahunAjaranOptionsTarget"],
    queryFn: async () => {
  ...
  const openDT = toDayjs(periodData.open_at);
  const closeDT = toDayjs(periodData.close_at);
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mt: -3 }}>
        {!canSubmit && (
          <Alert severity="warning" sx={{ mb: 2 }}>
            Periode dengan status <b>{status || "-"}</b> tidak dapat diedit.
          </Alert>
        )}

        {isOpen && (
          <Alert severity="info" sx={{ mb: 2 }}>
            Status <b>OPEN</b>: hanya boleh <b>memperpanjang waktu tutup</b> (close_at). Nama, target tahun ajaran, dan open_at dikunci.
          </Alert>
        )}
        ...
        <Grid container spacing={2} rowSpacing={1}>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            ...
            <CustomSelect
              ...
              value={periodData.tahun_ajaran_target_id || ""}
              onChange={handleChange}
              fullWidth
              required
              displayEmpty
              disabled={!isDraft || isTahunAjaranLoading || tahunAjaranError}
              inputProps={{ "aria-label": "Pilih Target Tahun Ajaran" }}
            >
...
export default PpdbPeriodEditForm;