...
const PegawaiGuruForm = ({ setSuccess, setError }) => {
    ...
    const { data: subKategoriOptions = [], isError: subKategoriError, refetch: refetchSubKategori } = useQuery({
        ...
        queryFn: async () => {
            if (!formState.kategori_pegawai_id) {
                return []; // Jangan panggil API jika kategori_pegawai_id kosong
            }
    ...
    const mutation = useMutation({
        mutationKey: ["tambahGuru"],
        mutationFn: async (newGuru) => {
            const response = await axiosInstance.post('/api/v1/admin-sekolah/pegawai', newGuru);
            return response.data;
        },
        onSuccess: (data) => {
            setSuccess(data.msg);
            setError("");
            setTimeout(() => navigate("/dashboard/admin-sekolah/pegawai/guru"), 3000);
        },
        onError: (error) => {
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: -2}}>
            <Box sx={{ borderRadius: 2 }}>
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                        <CustomFormLabel htmlFor="name" sx={{ mt: 1.85 }}>Nama Guru</CustomFormLabel>
                        <CustomOutlinedInput
                            id="name"
                            name="name"
...
export default PegawaiGuruForm;
