...
const getNamaMapel = (row) => {
  return row?.SubKategoriPegawai?.nama_subkategori || 'Tidak Ada';
};
...
const getTelepon = (row) => {
  const t = row?.nomor_telepon;
  return t && String(t).trim().length > 0 ? t : '-';
};
...
const PegawaiGuruTable = ({
  ...
  const safeGuru = Array.isArray(guru) ? guru : [];
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              (rowsPerPage > 0
                ? safeGuru.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                : safeGuru
              ).map((pegawaiGuru, index) => (
                <TableRow key={pegawaiGuru?.id ?? index}>
                  <TableCell>
                  ...
                  <TableCell align='center'>
                    <Typography sx={{ fontSize: '1rem' }}>
                      {getNamaMapel(pegawaiGuru)}
                    </Typography>
                  </TableCell>

                  <TableCell align='center'>
                    <Typography sx={{ fontSize: '1rem' }}>
                      {getTelepon(pegawaiGuru)}
                    </Typography>
                  </TableCell>
